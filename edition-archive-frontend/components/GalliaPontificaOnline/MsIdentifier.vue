<template>
  ...
  <span :class="`${popoverClass}`">
    ...
    <div v-if="model.show && model.manuscript"  class="popout text-start">
      ...
      <div class="link-list">
          <nuxt-link
            :to="`/gallia-pontifica-online/regesten/${route.params.regesten}/regest/suche/handschriften?handschriftenKey=${props.handschriftenKey}`">{{ $t('search.label') }}</nuxt-link>
          <nuxt-link
            :to="`/gallia-pontifica-online/regesten/${route.params.regesten}/regest/handschriften/#${props.handschriftenKey}`">{{ $t('gpo.pages.manuscriptIndex') }}</nuxt-link>
