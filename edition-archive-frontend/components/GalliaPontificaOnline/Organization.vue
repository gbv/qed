  ...
  <span class="popout-wrapper position-relative d-inline">
    ...
    <div v-if="model.show" class="popout text-start">
      ...
      <div v-if="model.loaded && model.organization" class="link-list">
          ...
          <nuxt-link :to="`/gallia-pontifica-online/regesten/${route.params.regesten}/regest/suche/organisation?organisationObj=${model.organization.id}`">{{$t('search')}}</nuxt-link>
          <nuxt-link :to="`/gallia-pontifica-online/regesten/${route.params.regesten}/regest/orte/#${model.organization.id}`">{{$t('regest_place_index')}}</nuxt-link>
      </div>
      <template v-else>
...
const loadOrganization = async (key?: string) => {
  ...
  model.show = true;
  if (!model.loaded) {
    const organizationSearchResp = await fetch(`${$solrURL()}main/select/?q=id:${key}%20AND%20objectType:organization&wt=json`);
