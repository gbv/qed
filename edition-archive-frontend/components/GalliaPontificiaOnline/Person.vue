  ...
  <span class="popout-wrapper position-relative d-inline">
    ...
    <div v-if="model.show" class="popout text-start">
      ...
      <div v-if="model.loaded && model.person" class="link-list">
          ...
          <nuxt-link :to="`/gallia-pontificia-online/regesten/${route.params.regesten}/regest/suche/person?personObj=${model.person.id}`">{{$t('search.label')}}</nuxt-link>
          <nuxt-link :to="`/gallia-pontificia-online/regesten/${route.params.regesten}/regest/personen/#${model.person.id}`">{{$t('gpo.pages.personIndex')}}</nuxt-link>
      </div>
      <template v-else>
...
const loadPerson = async (key?: string) => {
  ...
  model.show = true;
  if (!model.loaded) {
    const personSearchResp = await fetch(`${$solrURL()}main/select/?q=id:${escapeSpecialChars(key)}%20AND%20objectType:person&wt=json`);
