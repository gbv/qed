  ...
  <span class="popout-wrapper position-relative d-inline">
    ...
    <div v-if="model.show" class="popout text-start">
      ...
      <div v-if="model.loaded && model.place" class="link-list">
          ...
          <nuxt-link :to="`/gallia-pontificia-online/regesten/${route.params.regesten}/regest/suche/ort?ortObj=${model.place.id}`">{{$t('search.label')}}</nuxt-link>
          <nuxt-link :to="`/gallia-pontificia-online/regesten/${route.params.regesten}/regest/orte/#${model.place.id}`">{{$t('gpo.pages.placeIndex')}}</nuxt-link>
      </div>
      <template v-else>
